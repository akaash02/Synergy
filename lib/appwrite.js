...
export async function createTask(title, description, priority, dueDate, assignees) {
  try {
    const currentUser = await getCurrentUser();
...
export async function createMeeting(title, description, dueDate) {
  try {
    const currentUser = await getCurrentUser();
    if (!currentUser) throw new Error('No user logged in');
...
// Fetch and View User's Meetings
export async function viewMeetings() {
  try {
    const currentUser = await getCurrentUser();
    if (!currentUser) throw new Error('No user logged in');

    // Query to fetch all meetings created by the current user
    const userMeetings = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.meetingsCollectionId,
      [
        Query.equal('Creator', currentUser.accountId), // Match documents where Creator is the current user
      ]
    );

    return userMeetings.documents; // Return the list of meetings
  } catch (error) {
...
export async function searchUsersByUsername(query) {
  try {
    const users = await databases.listDocuments(
      ...
      appwriteConfig.userCollectionId,    // Your Users collection ID
      [Query.search("username", query)]    // Search for the 'username' field with the query
    );

    if (!users) throw new Error("Something went wrong");

    return users.documents; // Return the found user documents
  } catch (error) {
    throw new Error(error);
  }
}
